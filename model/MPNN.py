...
class MPNN(torch.nn.Module):
    def __init__(self,initpot,max_l=2,nwave=8,cutoff=4.0,ncontract=64,emb_nblock=1,emb_nl=[8,8],emb_layernorm=True,iter_loop=3,iter_nblock=1,iter_nl=[64,64],iter_dropout_p=[0.0,0.0],iter_layernorm=True,nblock=1,nl=[64,64],dropout_p=[0.0,0.0],layernorm=True,device=torch.device("cpu"),Dtype=torch.float32):
        ...
        emb_nl.insert(0,1)
        iter_nl.insert(0,self.ncontract)
        nl.insert(0,self.ncontract)
        ...
        initbias=torch.randn(nwave,device=device,dtype=Dtype)
    ...
    def density(self,orbital,cut_distances,iter_coeff,index_center,index_neigh,contracted_coeff,center_orbital):
        ...
        contracted_orbital=torch.einsum("ikj,kjm->kim",center_orbital,contracted_coeff)
        density=torch.einsum("kim,kim ->im",contracted_orbital,contracted_orbital)
